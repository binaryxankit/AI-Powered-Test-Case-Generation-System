...
PostgreSQL is not running, or the credentials in `backend/.env` are
wrong. Verify the connection string:

```powershell
psql "postgresql://testcase_user:testcase_pass@localhost:5432/testcase_ai"
```

The app still starts without a database (table creation is skipped
with a warning), but `POST /api/generate` and `GET /api/history` will
return 500 until the DB is reachable.

### `GEMINI_API_KEY is not configured`

Set `GEMINI_API_KEY` in `backend/.env` and restart the API. The key
is loaded once at startup.

### `ModuleNotFoundError: No module named 'backend'`

You launched uvicorn from inside the `backend/` directory. Run it
...
### Frontend shows "Network Error" or 500

* Check the browser dev-tools network tab. The error response has a
  `detail` field with the upstream message.
* Confirm `NEXT_PUBLIC_API_BASE_URL` in `frontend/.env.local` points
  at the running backend (default `http://localhost:8000`).
* CORS? The backend reads allowed origins from `CORS_ORIGINS`. Add
  your dev URL there if you changed the port.

### PDF endpoint returns HTML

You probably hit the backend through a proxy that intercepted the
`.pdf` extension. Request the URL directly and verify the
`Content-Type` header is `application/pdf`.

### Resetting the database

```powershell
psql -U testcase_user -d testcase_ai -c "DROP TABLE IF EXISTS test_generations CASCADE;"
python -m alembic upgrade head
```

The next request will recreate the table via the migration files.
